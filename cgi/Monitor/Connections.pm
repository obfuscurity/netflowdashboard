...
sub gather {
	...
	my $query = "SELECT DISTINCT agent_addr, src_addr, src_port, dst_addr, dst_port, protocol, if_index_in, if_index_out, ";
	...
	$query   .= "GROUP BY agent_addr, src_addr, dst_addr, src_port, dst_port, protocol, if_index_in, if_index_out ";
	$query   .= "ORDER BY $sort_field desc ";
	$query   .= "LIMIT ?";
my $debug_query = "EXPLAIN $query";
	my $sth = $dbh->prepare($query);
my $debug_sth = $dbh->prepare($debug_query);
	$sth->bind_param(5, ($cgi->param('c') || 10), { TYPE => DBI::SQL_INTEGER });
$debug_sth->bind_param(5, ($cgi->param('c') || 10), { TYPE => DBI::SQL_INTEGER });
	$sth->execute($cgi->param('d'), $then, $now, $cgi->param('i'), $cgi->param('i'), ($cgi->param('c') || 10)) || die $dbh->errstr;
$debug_sth->execute($cgi->param('d'), $then, $now, $cgi->param('i'), $cgi->param('i'), ($cgi->param('c') || 10)) || die $dbh->errstr;
$self->{'debug'} = $debug_sth->fetchall_arrayref;
	my @connections;
	...
	while (my $result = $sth->fetchrow_hashref) {
		...
		$result->{'time'} = $time;
		push(@connections, $result);
	}
	$self->{'list'} = \@connections;
	$self->{'subtitle'} = "Connections&nbsp;&nbsp;(Last $time hour/s)";
	return;
}
