...
sub gather {
	...
	$sth->bind_param(6, ($cgi->param('c') || 10), { TYPE => DBI::SQL_INTEGER });
	$sth->execute($cgi->param('h'), $cgi->param('d'), $then, $now, $cgi->param('i'), $cgi->param('i'), ($cgi->param('c') || 10)) || die $dbh->errstr;
	my @tophosts;
	...
	while (my $result = $sth->fetchrow_hashref) {
		...
		$result->{'time'} = $time;
		push(@tophosts, $result);
	}
	$self->{'list'} = \@tophosts;
	if ($cgi->param('host_side') eq 'src') {
		$self->{'subtitle'} = "Top Sources - " . $cgi->param('h') . "&nbsp;&nbsp;(Last $time hour/s)";
	} else {
		$self->{'subtitle'} = "Top Destinations - " . $cgi->param('h') . "&nbsp;&nbsp;(Last $time hour/s)";
	}
	return;
}
